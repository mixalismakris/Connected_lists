...
/*
    Σύστημα διαχείρισης βιβλιοθήκης μέσω κονσόλας.
  Στην εκκίνηση φορτώνει αυτόματα τη βιβλιοθήκη από το "library.txt" και
  εισέρχεται σε βρόχο μενού 12 επιλογών (0-11), καλώντας τις αντίστοιχες
  μεθόδους των κλάσεων Library και BookNode. Η έξοδος με επιλογή 0 ρωτά
  τον χρήστη αν θέλει να αποθηκεύσει πριν τον τερματισμό.
 */
...
int main() {
    ...
    while (choice != 0) {
        ...
        //Αν ο χρήστης εισάγει κάποιο μη έγκυρο ακέραιο, το cin μπαίνει σε failstate και δημιουργείται ατέρμονας βρόχος.
        if (!cin){
            cin.clear();    //καθαρίζουμε το failstate
            cin.ignore(10000,'\n'); //αγνοεί την είσοδο χαρακτήρων(δεν πρόκειται ο χρήστης να εισάγει 10000 χαρακτήρες)μέχρι την νέα γραμμη.
            cout<< "Λάθος είσοδος! Προσπαθήστε ξανά"<<endl;
            choice =-1; //μετα το failstate ο compiler θέτει την μεταβλητή προορισμού με 0,άρα θα τέλειωνε ο βρόχος
            continue;
        }
