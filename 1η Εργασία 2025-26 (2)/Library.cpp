...
void BookNode::addCopy(int copyid, string status) {       //Προσθέτει νέο αντίτυπο στην λίστα των αντιτύπων,με βάση το id.Η ταξινόμηση γίνεται κατά την εισαγωγή
    CopyNode* newCopy = new CopyNode(copyid, status);
    
    
    if (copiesHead == nullptr ||  copyid < copiesHead -> copyID ){// Ειδική περίπτωση: εισαγωγή στην αρχή (κενή λίστα ή μικρότερο copyID)
    ...
    {
        CopyNode* current = copiesHead; //Δημιουργούμε δείκτη με την διεύθυνση του τωρινού αντίγραφου,ώστε να διατρέξουμε την λίστα
        while (current -> next && current->next->copyID < copyid ){ // Διατρέχουμε όσο το επόμενο copyID είναι μικρότερο από το νέο                 
...
bool BookNode::borrowCopy(int copyid) {// αλλαγή κατάστασης αντιτύπου (με βάση το id) σε δανεισμένο αν είναι διαθέσιμο.Αν επιτυχής επιστρέφει true,ενώ αν δεν βρέθηκε ή δεν είναι διαθέσιμο false
...
void BookNode::removeCopy(int copyid) {// διαγραφή αντιτύπου με βάση το id απο την λίστα αντιτύπων.Ειδική περίπτωση η διαγραφή του πρώτου κόμβου.
    if (!copiesHead){ //αν η λίστα είναι κενή
        cout << "Δεν υπάρχουν αντίτυπα!"<< endl;
        return;
    }
    ...
    if (current ->copyID == copyid ){ //Αν αφαιρείται το πρώτο αντίγραφο
        if(current -> status == "borrowed"){ //είναι δανεισμένο
                cout <<"Το αντίτυπο έχει δανειστεί! Δεν μπορεί να διαγραφεί."<<endl;
                return;
            }
    ...
    while(current && current -> next){//όσο υπάρχει ο τωρινός κόμβος αλλα ΚΑΙ ο επόμενος, επανέλαβε
        if (current -> next -> copyID == copyid){ //ελέγχουμε τον επόμενο του τωρινόυ,για ευκολότερη αντιμετάθεση
            if(current -> next -> status == "borrowed"){ //είναι δανεισμένο
                cout <<"Το αντίτυπο έχει δανειστεί! Δεν μπορει να διαγραφεί."<<endl;
                return;
            }
            else{
...
void BookNode::returnCopy(int copyid) { //Επιστροφή αντιτύπου.Ρωτάει τον χρήστη για την κατάσταση του.
    CopyNode* current = copiesHead; // διάτρεξη λίστας από την αρχή
    while (current)//όσο υπάρχει κόμβος, επανέλαβε
    {
        if(current -> copyID == copyid){ //αν το copyid ισούται με αυτό που ψάχνουμε
            if (current -> status != "borrowed"){ //το αντίτυπο δεν είναι δανεισμένο
            ...
            while(true){//ώστε να γίνεται έλεγχος εγκυρότητας
                cout<<"Είναι το αντίτυπο σε καλή κατάσταση; (y/n)"<< endl; 
                cin >> answer;
                if (answer == "y" || answer == "Y" ){
                    current -> status = "available"; //αλλαγή κατάστασης σε διαθέσιμο
                    cout <<"Επιστράφηκε το αντίτυπο σε καλή κατάσταση." <<endl;
                    return;
                }
                else if (answer == "n" || answer == "N"){
                    current -> status = "damaged"; //αλλαγή κατάστασης σε κατεστραμένο
                    cout <<"Επιστράφηκε το αντίτυπο σε κακή κατάσταση." <<endl;
                    return;
                }
                else{
...
int BookNode::countAvailable() { //Διάτρεξη λίστας αντιτύπων και επιστροφή τον διαθέσιμων.
    int count = 0;
...
Library::Library() {  //αρχικοποίηση κενής βιβλιοθήκης
    booksHead = nullptr;
...
void Library::addBook(string title, string author, string isbn) {  //Δημιουργία νέου booknode και εισαγωγή του στη σωστή θέση της λίστας βιβλίων.
...
void Library::removeBook(string isbn) {  //διαγραφή βιβλίου απο την λίστα.
    ...
    while (current ){//όσο υπάρχει κόμβος κύριας λίστας 
        if (current -> ISBN == isbn){ //αν το isbn αντιστοιχεί
            ...
            while(currentOfCopies){ //όσο υπάρχουν αντίγραφα
                if(currentOfCopies -> status == "borrowed"){ //αν έστω και ένα βιβλίο έχει δανειστεί , η διαγραφή ακυρώνεται
                    cout << "Υπάρχει δανεισμένο αντίτυπο! Το βιβλίο δεν μπορεί να διαγραφεί"<<endl; 
                    return;
                }
            ...
            //εφόσον φτάσουμε εδώ, έχουμε αδειάσει από αντίγραφα και είμαστε έτοιμη για αντιγραγή βιβλίου
        ...
        previous = current; //αποθηκεύουμε τον τωρινό στη μεταβλητή previous ώστε να τον έχουμε μετά για αντιμετάθεση
...
BookNode* Library::findBook(string isbn) {// γραμμική αναζήτηση βιβλίου στην λίστα βιβλίων με βάση το isbn
...
void Library::printAll() { //εμφάνηση όλων των βιβλίων της βιβλιοθήκης(τίτλος,συγγραφέας,isbn,διαθέσιμα και συνολικά αντίτυπα.)
    ...
    while(current){ //όσο υπάρχει επόμενος κόμβος
            ...
            cout << "Αριθμός διαθέσιμων αντιτύπων: " << current -> countAvailable() << endl;
            ...
            cout << "Αριθμός συνολικών αντιτύπων: " << totalCopies << endl;
   ...
   while (getline(file, line)) { // Διαβάζουμε το αρχείο γραμμή προς γραμμή μέχρι να φτάσουμε στο τέλος του αρχείου
        ...
        if (tokens[0] == "BOOK") {      // Εάν το πρώτο token είναι "BOOK", σημαίνει ότι η γραμμή περιέχει πληροφορίες για ένα βιβλίο
            ...
            if (b) { // Εάν το βιβλίο βρέθηκε, προσθέτουμε το αντίγραφο στο βιβλίο χρησιμοποιώντας τη μέθοδο addCopy
                ...
                cout << "Σφάλμα: δεν βρέθηκε το βιβλίο με ISBN " << lastIsbn << " για να προσθέσουμε το αντίτυπο.\n";
            }
        }
    }
